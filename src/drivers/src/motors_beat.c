...
static uint16_t motorPowerSet[] = {0, 0, 0, 0, 0, 0}; // user-requested PWM signals (overrides)
static uint16_t motor_ratios[] = {0, 0, 0, 0, 0, 0};  // actual PWM signals

const uint16_t testsound[NBR_OF_MOTORS] = {A4, A5, F5, D5 , E5, F5};
...
// we will handle this in our power_distribution_beat.c file
...
void motorsStop()
{
  ...
  {
    motorsSetRatio(i, powerDistributionStopRatio(i));
  }
...
void motorsBurstDshot()
{
    ...
    motorMap[MOTOR_M1]->DMA_stream->NDTR = DSHOT_DMA_BUFFER_SIZE;
    TIM_DMACmd(motorMap[MOTOR_M1]->tim, motorMap[MOTOR_M1]->TIM_DMASource, ENABLE);
    DMA_ITConfig(motorMap[MOTOR_M1]->DMA_stream, DMA_IT_TC, ENABLE);
    DMA_Cmd(motorMap[MOTOR_M1]->DMA_stream, ENABLE);

    motorMap[MOTOR_M4]->DMA_stream->NDTR = DSHOT_DMA_BUFFER_SIZE;
...
LOG_ADD_CORE(LOG_UINT16, m2, &zero)
LOG_ADD_CORE(LOG_UINT16, m3, &zero)
...
LOG_ADD(LOG_UINT16, s1, &motor_ratios[MOTOR_S1])
LOG_ADD(LOG_UINT16, s2, &motor_ratios[MOTOR_S2])
LOG_ADD(LOG_UINT16, s3, &motor_ratios[MOTOR_S3])
LOG_ADD(LOG_UINT16, s4, &motor_ratios[MOTOR_S4])
